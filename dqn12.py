...
class Environment:
    ...
    def step(self, action):
        ...
        cpu_needed = tasks * CPU_USAGE_PER_TASK

        if action == 0:  # Process at edge
            if self.cpu_at_edge + cpu_needed > EDGE_CPU_LIMIT:
                reward = -high_penalty  # High penalty for overloading CPU at edge
            else:
                self.tasks_at_edge += tasks
                self.cpu_at_edge += cpu_needed
                reward = tasks  # Reward for processing at edge
        elif action == 1:  # Offload to fog
            if self.cpu_at_fog + cpu_needed > FOG_CPU_LIMIT:
                reward = -high_penalty  # High penalty for overloading CPU at fog
            else:
                self.tasks_at_fog += tasks
                self.cpu_at_fog += cpu_needed
                reward = -tasks * 0.1  # Lesser penalty for offloading to fog
        else:  # Offload to cloud
            self.tasks_at_cloud += tasks
            reward = -tasks * 0.2  # Penalty for offloading to cloud
