...
class CloudLayer:
    ...
    def process_task(self, task):
        self.nodes[0].queue.enqueue(task)
...
def simulate_processing_and_dequeue(layer):
    ...
    while True:
        print("queue : ====>" + str(layer.queue.is_empty()))
...
def concurrent_processing(edgelayer,foglayer,cloudlayer):
    threads = []

    tArrival = threading.Thread(target=simulate_TaskArrivalToEdge, args=(edgelayer,foglayer,cloudlayer))
    threads.append(tArrival)
    tArrival.start()
    
    for node in edgelayer.nodes:
        thread = threading.Thread(target=simulate_processing_and_dequeue, args=(node,))
        threads.append(thread)
        thread.start()
    for node in foglayer.nodes:
        thread = threading.Thread(target=simulate_processing_and_dequeue, args=(node,))
        threads.append(thread)
        thread.start()
    for node in cloudlayer.nodes:
        thread = threading.Thread(target=simulate_processing_and_dequeue, args=(node,))
        threads.append(thread)
        thread.start()
...
def simulate_TaskArrivalToEdge(edge_layer, fog_layer,cloud_layer):

    with open('C:\Dev\IMLETORMDC\RasberryPiSlice.csv', 'r') as file:
      ...
      for line_number, row in enumerate(reader, start=2):  # Start from line 2
          try:
              ...
              if start_time is None:
                  ...
                  dt1 = datetime.datetime.strptime(start_time, format_str)
                  dt2 = datetime.datetime.strptime(datetime_task, format_str)
...
# Function for concurrent processing
def concurrent_TaskArrivalToEdge():
    thread = threading.Thread(target=simulate_TaskArrivalToEdge)
    thread.start()    
    thread.join()
...
action_size = 3  # Three possible actions: Edge, Fog, Cloud
dqn = DQN(state_size, action_size)

# Concurrent processing
print("\nConcurrent Processing :")
concurrent_processing(edge_layer,fog_layer,cloud_layer)
