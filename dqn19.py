...
with open('/content/drive/MyDrive/Colab Notebooks/slice.csv', 'r') as file:
    ...
    for line_number, row in enumerate(reader, start=2):  # Start from line 2
        try:
            ...
            # Calculate delay based on the difference from the first task's request time
            current_time = time.time()
            expected_time = start_time + (request_time - first_request_time)
            delay = max(0, expected_time - current_time)
            time.sleep(delay)  # Delay enqueueing
